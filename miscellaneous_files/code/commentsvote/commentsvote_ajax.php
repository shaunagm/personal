...
{
	/*
		We have this extra abstraction on the input fields 
because during testing, we identified
		at least one other plugin that was using the same 
variable names and which was stealing
		our input.
	*/
	$fields = array(
...
function _CVAddVote($commentID, $userID, $vote, $ip, $time)
{
	...
	$alreadyVoted = $wpdb->get_var( $wpdb->prepare("SELECT count(*) 
FROM $cvTable WHERE commentID = %d  AND $userQuery[0]", $commentID, 
$userQuery[1]));
...
function CVDoAddVote($userID, $ip)
{
	...
	if( !$input ) 
		...
		if( $options['require_login'] && !$userID )
			$response['message'] = 'You must login to 
vote.';
		else {
			$voteRecorded = _CVAddVote( $commentID, $userID, 
$input['vote'], $ip, time() );

			if( !$voteRecorded )
				$response['message'] = 'You already 
responded.';
			else {
				if ($input['vote'] == -1)
				{
				...
				$formstring = "<a 
href='http://thehathorlegacy.com/wp-content/plugins/commentsvote/emailform.php?cID=" 
. $commentID . "' target='_blank'>Please click here to provide a reason 
for the flag.</a>";
...
recorded. Thank you.';
		...
		if( $options['display_rating'] )
			$response['votes'] = CVGetCommentVote( 
$commentID );
	}
	
	//Don't want to require PHP 5.2 quite yet, even though everyone 
